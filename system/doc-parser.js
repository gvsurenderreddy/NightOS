...
var NightElement = function(element, dom, doc) {
    ...
    /**
      ...
      * @return {string}
      */
    this.tag = function() { return element.tagName.toLocaleLowerCase(); };
    ...
    this.parent = function() {
        ...
        return p ? new NightElement(p, dom, doc) : this;
    };
    ...
    this.append = function(child) {
        //console.log(this.attr('id'), doc === docment, doc === udoc); // Result : private var `doc` is modified at one moment !!

        if(child instanceof Element) {
            if(doc.isSafeMode()) {
                /*
                var initOuterHTML = child.outerHTML;
                var parsed = doc.createElement('div').content(doc.safe(child.outerHTML));

                if(!parsed.length)
                    throw new Error('Element safety mismatch : Can append an unsafe element to a safe element only if it\'s main element tag is safe himself (div, span... / not script, meta...)');
                */

                if(child.outerHTML !== doc.safe(child.outerHTML))
                    throw new Error('Element safety mismatch : Can\'t append an unsafe element to a safe element');

                //console.log([parsed.content(), child.outerHTML, doc.safe(child.outerHTML)]);
            } //else console.log(this.attr('id'), doc === docment, doc === udoc)
    ...
    this.appendTo = function(parent) {
        if(parent instanceof Element)
            ...
            (new NightElement(parent, dom, doc)).append(element);
    ...
    /** Hide this element */
    this.hide = function() { this.css('display', 'none'); return this; };
    /** Show this element */
    this.show = function() { this.css('display', getElementDefaultDisplay(element.tagName)); return this; };
...
function NightCollection(dom, collection) {
    ...
    var doc, c = [], i, ex = new NightElement(document.createElement('div'), document.createElement('div'),
...
/**
  ...
  */
var NightDocument = function(content, safeMode, tagName, dontMark) {
    ...
    /**
      ...
      */
    this.tree = function() {
        return new NightElement(dom, null, this);
    };
    ...
    this.createElement = function(tagName) {
        return (!safeMode || NightDocument.UnsafeTags.indexOf(tagName) === -1) ? new NightElement(document.createElement(tagName), dom, this) : false;
    };

    /**
      * Create a NightElement representation for a DOM object
      * @param {Element} element Element to make a representation of
      * @return {NightElement} NightElement representation
      */
    this.make = function(element) {
        if(!(element instanceof Element))
            return false;

        return (!safeMode || NightDocument.UnsafeTags.indexOf(element.tagName) === -1) ? new NightElement(element, dom, this) : false;
    };
    ...
    this.parse = function(content, safeMode) {
        ...
        content = content
            ...
            // Special replaces, removed because they are useless now a securisation is done on the content (see above)
    ...
    this.safe = function(content) {
        ...
        var collection = tmp.querySelectorAll(NightDocument.UnsafeTags.join(','));
        for(i = 0; i < collection.length; i += 1)
            collection[i].remove();
    ...
    /**
      * Check if document is in safe mode
      * @return {boolean}
      */
    this.isSafeMode = function() {
        return safeMode;
    };
